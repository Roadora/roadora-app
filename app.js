...
(function(){
  ...
  const WC_STRIP_CARDS_V39651 = [
    { name:'Rastplatz Maasduinen', meta:'118 km vanaf start', rating:'Schoon', type:'WC', img:'assets/hero-routeplan.webp', chips:['Gratis','Parkeren','Koffie dichtbij'] },
    { name:'Service Koblenz', meta:'248 km vanaf start', rating:'Open', type:'WC + shop', img:'assets/hero-overview.webp', chips:['24/7','Shop','Verschoonplek'] },
    { name:'Raststätte Würzburg', meta:'418 km vanaf start', rating:'Open', type:'Rustplaats', img:'assets/hero-roadtrip.webp', chips:['Snelweg','Eten dichtbij','Parkeren'] },
    { name:'Autohof Nürnberg', meta:'558 km vanaf start', rating:'Open', type:'Comfort stop', img:'assets/hero-hotels.webp', chips:['Douche','Shop','24/7'] },
    { name:'Stop München Noord', meta:'685 km vanaf start', rating:'Open', type:'Laatste pauze', img:'assets/hero-diary.webp', chips:['Familie','Koffie','Parkeren'] }
  ];
  ...
  function renderWcStrip(){
    ...
    container.innerHTML =
      '<div class="rd-hotels-strip-shell-v39636 rd-hotels-fullcards-v39637 rd-hotels-swipeback-v39638 rd-wc-strip-shell-v39651">' +
        '<div class="rd-hotels-scroll-v39636 rd-wc-scroll-v39651" aria-label="WC stops langs je route">' +
          WC_STRIP_CARDS_V39651.map((wc, index)=>
            '<button type="button" class="rd-hotel-card-v39636 rd-wc-card-v39651" data-wc-index="'+index+'">' +
              '<span class="rd-hotel-rank-v39636 rd-wc-rank-v39651">'+(index+1)+'</span>' +
              '<span class="rd-hotel-photo-v39636 rd-wc-photo-v39651" style="background-image:url('+wc.img+')"></span>' +
              '<span class="rd-hotel-name-v39636 rd-wc-name-v39651">'+wc.name+'</span>' +
              '<span class="rd-hotel-rating-v39636 rd-wc-rating-v39651">WC · '+wc.rating+'</span>' +
              '<span class="rd-hotel-meta-v39636 rd-wc-meta-v39651">'+wc.meta+'</span>' +
            '</button>'
          ).join('') +
        '</div>' +
      '</div>';
  ...
  function renderWcPreview(index){
    const drawer = document.querySelector('#mapDrawer');
    if(!drawer) return;
    const wc = WC_STRIP_CARDS_V39651[index] || WC_STRIP_CARDS_V39651[0];
    closeHotelPreview();
    document.body.setAttribute('data-wc-preview','open');
    document.body.setAttribute('data-hotel-preview','open');
    const pop = document.createElement('div');
    pop.className = 'rd-hotel-preview-popover-v39644 rd-wc-preview-popover-v39651';
    pop.setAttribute('role','dialog');
    pop.setAttribute('aria-label','WC stop preview');
    pop.innerHTML =
      '<button type="button" class="rd-hotel-preview-close-v39644" aria-label="Sluiten">×</button>' +
      '<div class="rd-hotel-preview-photo-v39644 rd-wc-preview-photo-v39651" style="background-image:url('+wc.img+')">' +
        '<span class="rd-hotel-preview-count-v39644">'+(index+1)+' / '+WC_STRIP_CARDS_V39651.length+'</span>' +
      '</div>' +
      '<div class="rd-hotel-preview-body-v39644 rd-wc-preview-body-v39651">' +
        ...
        '<div class="rd-hotel-preview-meta-v39644">WC · '+wc.type+' · '+wc.rating+'</div>' +
        '<div class="rd-hotel-preview-chips-v39644">' +
          wc.chips.map(chip => '<span>'+chip+'</span>').join('') +
        '</div>' +
        '<p class="rd-hotel-preview-copy-v39644">Praktische korte pauzestop langs je route, handig voor gezinnen en lange ritten.</p>' +
  ...
  document.addEventListener("click", function(e){
    ...
    const wc = e.target.closest && e.target.closest(".rd-wc-card-v39651");
    if(wc){
      e.preventDefault();
      e.stopPropagation();
      const wcIndex = parseInt(wc.getAttribute('data-wc-index') || '0', 10) || 0;
      document.querySelectorAll(".rd-wc-card-v39651").forEach(function(item){
    ...
    const hotel = e.target.closest && e.target.closest(".rd-hotel-card-v39636:not(.rd-fuel-card-v39646):not(.rd-charge-card-v39647):not(.rd-food-card-v39648):not(.rd-discover-card-v39649):not(.rd-wc-card-v39651)");
    if(!hotel) return;
    e.preventDefault();
    e.stopPropagation();
