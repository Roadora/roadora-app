...
/* ===== Roadora centralized router v2 =====
   Eén stabiele router voor Home → Route → Map.
   Oude dubbele click/touch handlers zijn verwijderd.
*/
(function(){
  "use strict";
  ...
  function handleClick(event){
    ...
    const menuBtn = target.closest("#menuToggle");
    if (menuBtn) {
      event.preventDefault();
      return toggleMenu(event);
    }

    const scrim = target.closest("#menuScrim");
    if (scrim) {
      event.preventDefault();
      closeMenu();
      return false;
    ...
    const homeBtn = target.closest('[data-action="home"], #backHomeBtn');
    if (homeBtn) {
      event.preventDefault();
      return setScreen("home");
    }

    const routeBtn = target.closest('[data-action="route"]');
    if (routeBtn) {
      event.preventDefault();
      return setScreen("route");
    }

    const mapBtn = target.closest("#openMapBtn, .rPlan, [data-open-map]");
    if (mapBtn) {
      event.preventDefault();
    ...
    if (bottomNav) {
      ...
      const label = (bottomNav.textContent || "").trim().toLowerCase();

      if (label.includes("route")) return setScreen("route");
    ...
    if (target.closest("#mapScreen .grab")) {
      event.preventDefault();
      qs("#mapScreen .sheet")?.classList.remove("is-compact");
      return false;
    }
  ...
  window.RoadoraApp = {
    ...
    showRoute: () => setScreen("route"),
    showMap: () => setScreen("map"),
    closeMenu
  };
