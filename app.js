...
const markerRefs=[];
const liveGoogleEvLayer=L.layerGroup().addTo(map);
let liveGoogleEvStops=[];
let liveGoogleEvLoaded=false;
let liveGoogleEvLoading=false;
...
function renderMarkers(){
  ...
  renderLiveGoogleEvMarkers();
  if(activeFilters.has('all') || activeFilters.has('ev')) loadLiveGoogleEvStations();
}

function currentRouteSamplePoints(maxPoints=5){
  const latlngs = routeMain && routeMain.getLatLngs ? routeMain.getLatLngs() : [];
  const source = latlngs && latlngs.length ? latlngs : route.map(p=>L.latLng(p[0],p[1]));
...
function renderLiveGoogleEvMarkers(){
  liveGoogleEvLayer.clearLayers();
  if(!(activeFilters.has('all') || activeFilters.has('ev'))) return;

  liveGoogleEvStops.forEach(s=>{
    const marker=L.marker(s.ll,{icon:makeStopIcon(s,false,false)}).addTo(liveGoogleEvLayer);
    marker.on('click',()=>{
      updateSheet(s);
...
async function loadLiveGoogleEvStations(){
  if(liveGoogleEvLoaded || liveGoogleEvLoading) return;
  liveGoogleEvLoading=true;

  try{
    showToast('Echte laadpalen zoeken…');

    const points=currentRouteSamplePoints(5);
    const res=await fetch('/api/google-ev', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({points, radiusMeters:18000})
    });

    const data=await res.json();
    if(!res.ok) throw new Error(data.error || 'Google EV API fout');

    liveGoogleEvStops=(data.places || []).map(p=>({
      name:p.name || 'Laadstation',
      meta:[
        p.address || 'Langs je route',
        p.rating ? `${p.rating} ★` : '',
        p.openNow === true ? 'Nu open' : ''
      ].filter(Boolean).join(' · '),
      desc:'Echt laadstation gevonden via Google Places langs je route. Beschikbaarheid/laadvermogen altijd controleren in Maps of laadapp.',
      type:'ev',
      label:'Google laadstation',
      ll:[p.lat,p.lng],
      provider:'Google Places',
      power:p.evOptions || 'laadinfo via Google',
      status:p.openNow === true ? 'nu open' : 'check beschikbaarheid',
      googlePlaceId:p.id || null
    }));

    liveGoogleEvLoaded=true;
    liveGoogleEvLoading=false;
    renderLiveGoogleEvMarkers();
    showToast(liveGoogleEvStops.length ? `${liveGoogleEvStops.length} echte Google laadpalen` : 'Geen echte laadpalen gevonden');
  }catch(err){
    liveGoogleEvLoading=false;
    console.warn('Live Google laadpalen fout:', err);
    showToast('Google laadpalen niet geladen');
  }
}
...
async function loadOrsRoute(){
  try{
    ...
    liveGoogleEvLoaded=false; liveGoogleEvStops=[]; liveGoogleEvLayer.clearLayers();
    if(activeFilters.has('all') || activeFilters.has('ev')) loadLiveGoogleEvStations();
