...
(function(){
  ...
  function handleClick(event){
    ...
    if(bottomNav){event.preventDefault();setActiveBottomNav(bottomNav);const label=(bottomNav.textContent||'').trim().toLowerCase();if(label.includes('route')) return setScreen('route');if(label.includes('overzicht')){setSheet('overview');return false;}if(label.includes('navigeer')){openMapsRoute();return false;}if(label.includes('stops')){setSheet('stops');return false;}if(label.includes('reisgids')){setSheet('guide');return false;}return false;}
...
(function(){
  ...
  window.initRoadoraMapSubpage=function(){
    ...
    function renderMarkers(){
      markerLayer.clearLayers();markerRefs.length=0;
      stops.forEach(s=>{if(!isVisible(s)) return;const marker=L.marker(s.ll,{icon:makeStopIcon(s,false,false)}).addTo(markerLayer);const ref={marker,stop:s};markerRefs.push(ref);marker.on('click',()=>selectStop(ref,true));});
      if(selectedMarker && !isVisible(selectedMarker.stop)){selectedMarker=null;updateSheet(destinationSheet);}
    ...
    function bootMap(){
      if(mapBooted){map.invalidateSize(true);fit();return;}
      mapBooted=true;
      map.invalidateSize(true);updateSheet(destinationSheet);renderMarkers();syncCatUI();fit();loadOrsRoute();
    }

    window.RoadoraMapApi={
      setFilters,
      reloadRoute:()=>{mapBooted=false;loadOrsRoute();},
      fitRoute:fit,
      showPanel:updateSheet,
    ...
    document.getElementById('fitRoute')?.addEventListener('click',()=>{fit();showToast('Volledige route in beeld');});
