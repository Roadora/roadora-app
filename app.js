...
(function(){
  ...
  function handleClick(event){
    ...
    if(bottomNav){event.preventDefault();setActiveBottomNav(bottomNav);const label=(bottomNav.textContent||'').trim().toLowerCase();if(label.includes('route')){window.RoadoraMapApi?.clearSelection?.();window.RoadoraMapApi?.fitRoute?.('nav');toast('Volledige route in beeld');return false;}if(label.includes('overzicht')){setSheet('overview');return false;}if(label.includes('navigeer')){openMapsRoute();return false;}if(label.includes('stops')){window.RoadoraMapApi?.toggleCategories?.();return false;}if(label.includes('reisgids')){setSheet('guide');return false;}if(label.includes('meer') || bottomNav.dataset.nav==='more'){return toggleMenu(event);}return false;}
...
/* Roadora v5.3 — Hotel Tab Overnachtingsplanner
   ...
   - Reisfilters: huisdier, kindvriendelijk, laadpaal, parkeren, ontbijt, late check-in
...
/* Roadora v5.7 Interactielaag v1
   ...
   - Voorbereid op toekomstige reis-subpage
*/
(function(){
  'use strict';
  const qs=(s,r=document)=>r.querySelector(s);
...
/* Roadora clean Maps export shim */
(function(){
  'use strict';
  if(window.RoadoraMapsExport?.open) return;
  const DEST='Innsbruck, Oostenrijk';
  function url(){
    const params=new URLSearchParams({api:'1',destination:DEST,travelmode:'driving'});
    return 'https://www.google.com/maps/dir/?'+params.toString();
  }
  function open(){ window.open(url(),'_blank','noopener'); return false; }
  window.RoadoraMapsExport={url,open};
})();


/* Roadora — Meer knop fallback fix */
(function(){
  if(window.__roadoraMeerFixBound) return;
  window.__roadoraMeerFixBound = true;
  document.addEventListener('click', function(event){
    const btn = event.target && event.target.closest && event.target.closest('#mapScreen .bottomNav .navItem[data-nav="more"]');
    if(!btn) return;
    event.preventDefault();
    event.stopPropagation();
    ...
    const isOpen = phone && (phone.classList.contains('menuOpen') || phone.classList.contains('menuExpanded'));
    if(isOpen){
      phone.classList.remove('menuOpen','menuExpanded');
      sideMenu && sideMenu.classList.remove('open','active','show');
      scrim && scrim.classList.remove('open','active','show');
    }else{
      phone && phone.classList.add('menuOpen','menuExpanded');
      sideMenu && sideMenu.classList.add('open','active','show');
      scrim && scrim.classList.add('open','active','show');
    }
    return false;
  }, true);
})();
