...
/* Roadora v39.7.47 — map active nav truth guard.
   When the Stops/Nu nodig/Meer sheet is open, the active tab must come from
   data-instant-map-panel. This prevents Roadtrip from staying highlighted due
   to older data-map-panel handlers. No ORS, map drawing or route-loading logic touched. */
(function(){
  if(window.__roadoraMapNavTruthV39747) return;
  window.__roadoraMapNavTruthV39747 = true;

  function normalizePanel(panel){
    panel = String(panel || '').trim();
    return panel === 'stops' || panel === 'now' || panel === 'more' || panel === 'roadtrip' ? panel : 'roadtrip';
  }

  function currentPanel(){
    var drawerOpen = document.body.getAttribute('data-map-drawer') === 'open';
    var instant = document.body.getAttribute('data-instant-map-panel');
    if(drawerOpen && instant) return normalizePanel(instant);
    return normalizePanel(document.body.getAttribute('data-map-panel') || 'roadtrip');
  }

  function sync(panel){
    panel = normalizePanel(panel || currentPanel());
    document.body.setAttribute('data-map-panel', panel);
    document.querySelectorAll('body > nav.rd-map-nav-v28 .rd-nav-btn-v28').forEach(function(btn){
      var btnPanel = normalizePanel(btn.dataset && btn.dataset.mapPanel);
      var active = btnPanel === panel;
      btn.classList.toggle('is-active', active);
      btn.classList.remove('active');
      if(active) btn.setAttribute('aria-current','page');
      else btn.removeAttribute('aria-current');
  ...
  document.addEventListener('click', function(e){
    ...
    if(!btn) return;
    var panel = normalizePanel(btn.dataset && btn.dataset.mapPanel);
    setTimeout(function(){ sync(panel); }, 0);
    setTimeout(function(){ sync(); }, 60);
    setTimeout(function(){ sync(); }, 180);
  }, true);

  var observer = new MutationObserver(function(muts){
    for(var i=0;i<muts.length;i++){
      var name = muts[i].attributeName;
      if(name === 'data-map-drawer' || name === 'data-instant-map-panel' || name === 'data-map-panel'){
        sync();
        break;
      }
    }
  });
  try{ observer.observe(document.body, { attributes:true }); }catch(_){ }
