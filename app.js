...
/* ===== Roadora Google Places Autocomplete v1 — safe integration ===== */
(function(){
  'use strict';

  window.roadoraPlaces = window.roadoraPlaces || {
    origin: null,
    destination: null,
    homeSearch: null
  };
  ...
  function toast(txt){
    const t=document.querySelector('#mapToast');
    if(t){
      t.textContent=txt;
      t.classList.add('show');
      clearTimeout(toast._t);
  ...
  async function selectedToPlace(event){
    if(!event || !event.placePrediction) return null;
  ...
  function mountIntoRouteInput(inputEl, id, placeholder){
    if(!inputEl || inputEl.dataset.rdPlacesMounted) return null;
    inputEl.dataset.rdPlacesMounted='1';
    inputEl.classList.add('has-google-autocomplete');
    inputEl.innerHTML = '<div id="'+id+'" class="rdGoogleAutocomplete"></div>';
  ...
  window.initRoadoraAutocomplete = async function(){
    ...
    try{
      ...
      const routeInputs = Array.from(document.querySelectorAll('#routeSetupScreen .rInput'));
      const originHolder = mountIntoRouteInput(routeInputs[0], 'roadoraOriginAutocomplete', 'Vertrekpunt');
      const destHolder = mountIntoRouteInput(routeInputs[1], 'roadoraDestinationAutocomplete', 'Bestemming');

      if(originHolder && !originHolder.dataset.ready){
        originHolder.dataset.ready='1';
        const origin = new PlaceAutocompleteElement({
          placeholder: 'Vanwaar vertrek je?',
          requestedLanguage: 'nl',
          requestedRegion: 'nl'
        });
        originHolder.appendChild(origin);
        origin.addEventListener('gmp-select', async (event)=>{
          window.roadoraPlaces.origin = await selectedToPlace(event);
          toast('Vertrekpunt gekozen');
          console.log('Roadora vertrekpunt:', window.roadoraPlaces.origin);
        });
      }

      if(destHolder && !destHolder.dataset.ready){
        destHolder.dataset.ready='1';
        const dest = new PlaceAutocompleteElement({
          placeholder: 'Waar wil je heen?',
          requestedLanguage: 'nl',
          requestedRegion: 'nl'
        });
        destHolder.appendChild(dest);
        dest.addEventListener('gmp-select', async (event)=>{
          window.roadoraPlaces.destination = await selectedToPlace(event);
          toast('Bestemming gekozen');
          console.log('Roadora bestemming:', window.roadoraPlaces.destination);
        });
      }

      const homeInput = document.querySelector('.search input');
      if(homeInput && !homeInput.dataset.rdPlacesHint){
        homeInput.dataset.rdPlacesHint='1';
        homeInput.placeholder='Waar wil je naartoe?';
      }

      console.log('Roadora Google Places autocomplete actief');
    }catch(err){
      console.warn('Roadora autocomplete kon niet starten:', err);
    }
  };

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', ()=>{
      if(window.RoadoraGoogleMapsReady) window.initRoadoraAutocomplete();
    }, {once:true});
  }else{
    if(window.RoadoraGoogleMapsReady) window.initRoadoraAutocomplete();
  }
