...
/* ===== Roadora vanilla selector helpers ===== */
...
(function(){
  ...
  function openMapsRoute(){
    ...
    try{
      const params=new URLSearchParams({api:'1',destination:'Innsbruck, Oostenrijk',travelmode:'driving'});
  ...
  function openHotelDetail(){
    ...
    body.innerHTML=`
      ...
      <div class="hotelDetailActions">
        ...
        <button class="hotelGhost" data-hotel-action="save"></button>
      </div>
...
/* ===== ROADORA PREMIUM SUBPAGES V1 ROUTER ===== */
(function(){
  'use strict';
  const qs=(s,r=document)=>r.querySelector(s);
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  function closeMenu(){const p=qs('.phone');p?.classList.remove('menuOpen','menuExpanded');qsa('#sideMenu,#menuScrim').forEach(el=>el.classList.remove('open','active','show'));}
  function hidePremium(){qsa('.premiumScreen').forEach(s=>s.classList.remove('active'));}
  function showPremium(name){const p=qs('.phone');closeMenu();p?.classList.remove('mapActive');qs('#routeSetupScreen')?.classList.remove('active');qs('#mapScreen')?.classList.remove('active');hidePremium();qs('#'+name+'Screen')?.classList.add('active');qsa('.sideItem').forEach(i=>i.classList.toggle('active',i.dataset.action===name));}
  function showHome(){closeMenu();hidePremium();window.RoadoraApp?.showHome?.();}
  function showMap(){closeMenu();hidePremium();window.RoadoraApp?.showMap?.();}
  document.addEventListener('click',function(e){
    const nav=e.target.closest('[data-premium-nav]');
    if(nav){e.preventDefault();const n=nav.dataset.premiumNav;if(n==='home')return showHome();if(n==='map')return showMap();if(n==='roadtrip')return showPremium('roadtrip');if(n==='dagboek')return showPremium('dagboek');if(n==='profiel')return showPremium('profiel');}
    const side=e.target.closest('.sideItem');
    if(side){const a=side.dataset.action;if(a==='routes'){e.preventDefault();e.stopPropagation();return showPremium('roadtrip')}if(a==='account'){e.preventDefault();e.stopPropagation();return showPremium('profiel')}}
    const mem=e.target.closest('[data-premium-action="new-memory"]');if(mem){e.preventDefault();window.RoadoraToast?.('Nieuw moment koppelen we later aan galerij/notities');return}
    const day=e.target.closest('[data-premium-action="open-day"]');if(day){e.preventDefault();window.RoadoraToast?.('Dagdetails koppelen we later aan echte route-data');return}
  },true);
