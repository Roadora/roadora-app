...
const markerRefs=[];
const googleEvLayer=L.layerGroup().addTo(map);
let googleEvLoaded=false;
let googleEvLoading=false;
let googleEvStops=[];
...
function shouldShowGoogleEv(){
  return activeFilters.has('all') || activeFilters.has('ev');
}

function clearGoogleEvMarkers(){
  if(googleEvLayer) googleEvLayer.clearLayers();
}

function updateGoogleEvMarkersVisibility(){
  clearGoogleEvMarkers();
  if(!shouldShowGoogleEv()) return;

  googleEvStops.forEach(s=>{
    const marker=L.marker(s.ll,{icon:makeStopIcon(s,false,false)}).addTo(googleEvLayer);
    marker.on('click',()=>{
      updateSheet(s);
      map.flyTo(s.ll,8,{duration:.65});
    });
  });
}

function waitForGooglePlaces(timeoutMs=4500){
  return new Promise((resolve,reject)=>{
    ...
    const tick=()=>{
      ...
      if(Date.now()-start>timeoutMs){
        reject(new Error('Google Places niet geladen'));
        return;
      }
      setTimeout(tick,120);
    };
...
function routeSamplePointsForGoogle(count=5){
  const latlngs=routeMain.getLatLngs && routeMain.getLatLngs().length ? routeMain.getLatLngs() : route.map(p=>L.latLng(p[0],p[1]));
  if(!latlngs || latlngs.length<2) return [];

  const result=[];
  for(let i=1;i<=count;i++){
    const idx=Math.floor((latlngs.length-1)*(i/(count+1)));
    const p=latlngs[Math.max(0,Math.min(latlngs.length-1,idx))];
    result.push({lat:p.lat,lng:p.lng});
  }
  return result;
}

function nearbyEvSearch(service, point){
...
async function loadGoogleEvChargersOnce(){
  if(googleEvLoaded || googleEvLoading) return;
  googleEvLoading=true;

  try{
    await waitForGooglePlaces();

    const serviceHost=document.createElement('div');
    serviceHost.style.display='none';
    document.body.appendChild(serviceHost);

    const service=new google.maps.places.PlacesService(serviceHost);
    const points=routeSamplePointsForGoogle(5);
    const byId=new Map();

    for(const point of points){
      const results=await nearbyEvSearch(service,point);
      results.slice(0,6).forEach(place=>{
        const loc=place.geometry && place.geometry.location;
        const id=place.place_id || `${place.name}-${loc?.lat?.()}-${loc?.lng?.()}`;
        if(!loc || byId.has(id)) return;

        byId.set(id,{
          name:place.name || 'Laadstation',
          meta:[
            place.vicinity || 'Langs je route',
            place.rating ? `${place.rating} ★` : '',
            place.opening_hours && place.opening_hours.open_now ? 'Nu open' : ''
          ].filter(Boolean).join(' · '),
          desc:'Live laadstation gevonden via Google Places langs je route. Controleer beschikbaarheid altijd in je navigatie of laadapp.',
          type:'ev',
          label:'Google laadstation',
          ll:[loc.lat(),loc.lng()],
          provider:'Google Places',
          power:'check provider',
          status:place.opening_hours && place.opening_hours.open_now ? 'nu open' : 'beschikbaarheid checken',
          googlePlaceId:place.place_id || null
        });
      });
    }

    googleEvStops=Array.from(byId.values()).slice(0,10);
    googleEvLoaded=true;
    googleEvLoading=false;
    updateGoogleEvMarkersVisibility();

    if(googleEvStops.length){
      showToast(`${googleEvStops.length} echte laadstations geladen`);
    }else{
      showToast('Geen Google laadstations gevonden');
    }
  }catch(err){
    googleEvLoading=false;
    console.warn('Google EV laadstations niet geladen:',err);
    showToast('Google laadstations niet beschikbaar');
  }
}

window.RoadoraMapReadyForGooglePlaces=function(){
  if(shouldShowGoogleEv()) loadGoogleEvChargersOnce();
};
