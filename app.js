...
let currentRouteLatLngs=route.slice();
let routeDistanceKm=1005;
let stops=[
 {name:'Stuttgart route restaurant',meta:'± 510 km · eten langs route',desc:'Rustige eetstop dichtbij de route. Later wordt dit vervangen door echte restaurantdata.',type:'food',label:'Eten & drinken',ll:[48.77,9.18]},
 {name:'Hotel bij Ulm',meta:'± 650 km · overnachten langs route',desc:'Rustige overnachtingsplek dichtbij de route, handig voor een tweedaagse rit naar Innsbruck.',type:'hotel',label:'Overnachten',ll:[48.40,10.00]},
 {name:'Alpen uitzicht',meta:'± 900 km · korte pauze',desc:'Rustige scenic stop richting Oostenrijk, ideaal voor foto’s en een korte pauze.',type:'view',label:'Activiteit',ll:[47.42,11.12]}
];
...
function pointAtDistance(latlngs,targetKm){
  let travelled=0;
  for(let i=1;i<latlngs.length;i++){
    const a=latlngs[i-1], b=latlngs[i];
    const seg=kmBetween(a,b);
    if(travelled+seg>=targetKm){
      const t=(targetKm-travelled)/Math.max(seg,0.0001);
...
function offsetPoint(p,index){
  const side=index%2===0?1:-1;
  // heel kleine offset zodat pins niet exact bovenop de route liggen
  return [p[0]+side*0.045, p[1]-side*0.035];
}
function chargerNameFor(p,index){
  const lng=p[1], lat=p[0];
  if(lng<6.7) return ['Fastned Venlo Route','Shell Recharge Maas-Rijn','Allego Limburg Corridor'][index%3];
  if(lng<8.4) return ['EnBW Rhein-Main Ladepark','IONITY Frankfurt West','Aral pulse Rhein route'][index%3];
  if(lng<9.7) return ['EnBW Stuttgart Route','Fastned Schwabenpark','Shell Recharge A8 corridor'][index%3];
  if(lng<10.7) return ['IONITY Ulm Süd','EnBW Memmingen Ladepark','Allego Allgäu Route'][index%3];
  return ['IONITY Fernpass','Shell Recharge Tirol Poort','Allego Innsbruck Noord'][index%3];
}
function travelTimeText(km){
  const min=Math.round(km/92*60);
  return `${Math.floor(min/60)}u ${String(min%60).padStart(2,'0')}m`;
}
function buildRouteAwareChargers(latlngs,totalKm){
  const usable=Math.max(totalKm||routeDistanceKm||1000,300);
  const count=Math.max(5,Math.min(9,Math.round(usable/145)));
  const startGap=Math.min(135,usable*0.16);
  const endGap=Math.min(95,usable*0.10);
  const span=usable-startGap-endGap;
  const providers=['Fastned','EnBW','IONITY','Shell Recharge','Allego','Aral pulse'];
  return Array.from({length:count},(_,i)=>{
    const d=startGap+(span*(i/(count-1)));
    const routePoint=pointAtDistance(latlngs,d);
    const ll=offsetPoint(routePoint,i);
    const stalls=[8,12,6,10,4,14,8,6,10][i%9];
    const provider=providers[i%providers.length];
    return {
      name: chargerNameFor(routePoint,i),
      meta:`± ${Math.round(d)} km · ${travelTimeText(d)} · ${stalls} snelladers`,
      desc:'Route-aware laadstop langs je echte ORS-route. Dit is de juiste architectuur: pins worden verdeeld over de routecorridor en later vervangen door live laadpaaldata.',
      type:'ev',
      label:'Laadstation',
      ll,
      provider,
      power:i%3===0?'tot 350 kW':(i%3===1?'tot 300 kW':'tot 150 kW'),
      status:[`${Math.max(2,stalls-3)} vrij`,`${Math.max(1,stalls-5)} vrij`,'beschikbaar'][i%3]
    };
  });
}
function rebuildRouteStops(latlngs,totalKm){
  currentRouteLatLngs=latlngs.slice();
  routeDistanceKm=totalKm || routeDistanceKm;
  const nonEv=stops.filter(s=>s.type!=='ev');
  stops=[...buildRouteAwareChargers(currentRouteLatLngs,routeDistanceKm),...nonEv];
  renderMarkers();
}
...
async function loadOrsRoute(){
  try{
    ...
    const rawKm=summary.distance ? summary.distance/1000 : null;
    const km=rawKm ? Math.round(rawKm).toLocaleString('nl-NL')+' km' : null;
...
function bootMap(){
  ...
  updateSheet(destinationSheet);
  rebuildRouteStops(currentRouteLatLngs,routeDistanceKm);
  syncCatUI();
  fit();
  loadOrsRoute();
}
