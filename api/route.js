export default async function handler(req, res) {
  ...
  try {
    ...
    const rawVia = String(q.waypoints || '').trim();
    ...
    if (coordinates.length < 2) return res.status(400).json({ ok:false, error:'Ongeldige route-coordinaten' });
